<!-- Back to Top Button -->
<button id="backToTop" class="fixed bottom-5 right-5 bg-red-600 text-white p-3 rounded-full shadow-lg hover:bg-red-700 transition-opacity duration-300 opacity-0 hidden z-50">
    ↑
</button>
...
<header id="navbar" class="bg-gray-900 text-white fixed top-0 left-0 w-full z-50 py-4 transition duration-300">
    <nav class="relative max-w-7xl mx-auto px-4 flex justify-between items-center">
        <div class="flex items-center space-x-4">
            <img src="{{ asset('assets/logo/logo2.png') }}" alt="Logo" class="h-10 w-10 rounded-full object-cover">
            ...
            @auth()
                <li class="">
                    ...
                    <div class="absolute myhover mt-2 w-44 bg-gray-800 text-white border border-gray-200 rounded-md shadow-lg opacity-0 invisible transition-all duration-200 ease-in-out hover-trigger:hover:opacity-100 hover-trigger:hover:visible">
                        <a href="{{ route('profile.edit')}}" class="block w-full text-center px-4 py-2 text-sm text-white hover:bg-gray-600">Profile</a>

                        <form action="{{route('logout')}}" method="POST">
                            @csrf
                            <button type="submit" class="w-full px-4 py-2 text-sm text-white hover:bg-gray-600">Logout</button>
                        </form>
                    </div>
                </li>
            @else
